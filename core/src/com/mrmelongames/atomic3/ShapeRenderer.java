...
public class ShapeRenderer implements Disposable {
    ...
    /** Calls {@link #arc(float, float, float, float, float, int)} by estimating the number of segments needed for a smooth arc. */
    public void arcNoLegs (float x, float y, float radius, float start, float degrees) {
        arcNoLegs(x, y, radius, start, degrees, Math.max(1, (int)(6 * (float)Math.cbrt(radius) * (degrees / 360.0f))));
    }
    ...
    /** Draws an arc using {@link ShapeType#Line} or {@link ShapeType#Filled}. */
    public void arcNoLegs (float x, float y, float radius, float start, float degrees, int segments) {
        if (segments <= 0) throw new IllegalArgumentException("segments must be > 0.");
        float colorBits = color.toFloatBits();
        float theta = (2 * MathUtils.PI * (degrees / 360.0f)) / segments;
        float cos = MathUtils.cos(theta);
